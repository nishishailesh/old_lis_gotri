...
$r='select sign, sign_time_last from sample_verification where sample_id=\''.$_POST['sample_id'].'\'';
...
if($r2['sign']=='Verification_pending')
	...
	if($tm_diff < 3)	// set desired time in hours here, by which report can be edited by the signing person
		{
		update_sample($_POST);
		edit_sample($_POST['sample_id'],'edit_request_step_3.php','disabled');
		input_result($_POST['sample_id'],'edit_request_step_5.php' ,' ');
