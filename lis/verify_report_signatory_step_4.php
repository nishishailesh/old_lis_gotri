...
exit();
	...
	echo '<table  border=1>	
		<caption>Search reports awaiting verification between two sample ID</caption><form method=post action=\'verify_report_signatory_step_4.php\'>';
...
if(isset($_POST['submit']) && isset($_POST['from_sample_id']) && isset($_POST['to_sample_id']))
{
	...
	if(mysql_num_rows($result=mysql_query($sql,$link))>0)
	{
		...
		echo '<h3>The following reports between sample_id '.$_POST['from_sample_id'].' and '.$_POST['to_sample_id'].' awaits verification by technician:</h3>';
		echo '<table border=1><tr><td align=center><b>Sample_id</b></td></tr>';
	
		while($post_array=mysql_fetch_assoc($result))
		{
				
			echo '<tr><td>'.$post_array['sample_id'].'</td></tr>';
