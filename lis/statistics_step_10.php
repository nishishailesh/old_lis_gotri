...
echo '<tr><th colspan=35 bgcolor=lightgreen style=\'font-size:110%\'> COVID Test statistics for the month of '.$month_name.' 20'.$yy.'</th></tr>';
...
//echo '</pre>';
	...
	foreach ($ar as $key => $value)   //for every parameter in scope
	{
		...
		for ( $dd=01; $dd<=$entered_month_days ; $dd ++) 
		{
			...
			$p1= 'SELECT count(name_of_examination) FROM `examination` WHERE `name_of_examination`=\''.$ar['name_of_examination'].'\' and `sample_id` between \''.$yymm.$dd.'3001\' and \''.$yymm.$dd.'4000\' ';
		...
		//$q1= 'SELECT count(name_of_examination) FROM `examination` WHERE `name_of_examination`=\''.$ar['name_of_examination'].'\' and `sample_id` between \''.$yymm.'013000\' and \''.$yymm.'314000\' ';
