...
output_folder='/root/rochedata/' #remember ending/
alarm_time=10
...
def get_port():
  if(connection_type=='tty'):
    try:
      ...
      logging.debug('port exist. But, This does not mean that connection to opposite end is made')
      return port
    except:
      exception_return = sys.exc_info()
      logging.debug(exception_return)
