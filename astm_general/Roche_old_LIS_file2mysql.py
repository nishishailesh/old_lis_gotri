...
###### Changed by Shailesh ######
#import astm_var_gotri as astm_var
import astm_var
...
inbox='/root/rochedata/'
archived='/root/rochearch/'
...
class old_LIS(astmg.astm_file):
  def send_to_mysql(self):
    ...
    for each_sample in self.final_data:
      received_sample_id=each_sample[0]
      logging.debug("received_sample_id={}".format(received_sample_id))
      sample_id=received_sample_id.split("^")[1].rstrip(' ')
      logging.debug("===>Roche finalized sample_id={}".format(sample_id))
      ...
      for each_record in each_sample[1]:
        ...
        if(each_record[0]=='R'):
          if(each_record[1]=='1'):  #ignore if '2' (specific for abbot architect machines, '2' have absorbance, '1' have results)
            received_ex_code=each_record[2].split(self.s3)[3]
            logging.debug("received_ex_code={}".format(received_ex_code))
            ex_code=received_ex_code.split("/")[0]
            logging.debug("roche final ex_code={}".format(ex_code))
